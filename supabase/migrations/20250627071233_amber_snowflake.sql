/*
  # Admin System Setup - Fixed Migration

  This migration creates the admin system tables and functions with proper error handling.
  It avoids syntax errors by properly formatting the DO blocks and nested functions.
*/
...
-- Update questions table RLS policies for admin access
DO $$
BEGIN
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'questions') THEN
        EXECUTE 'CREATE POLICY "Users can view published questions or admins can view all"
          ON questions
          FOR SELECT
          TO authenticated
          USING (
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'questions') THEN
        EXECUTE 'CREATE POLICY "Admins can manage all questions"
          ON questions
          FOR ALL
          TO authenticated
          USING (
