...
The user has decided to consolidate on AWS with another project they run. This spec re-targets the same park-mode plan to AWS-native services. The product behavior, observe-window stage gates, content layer, and analytics goals from the 2026-05-22 spec are unchanged. Only the hosting and data layer move.
...
- Keep monthly steady-state cost ≤ $20/mo
...
| D5 | Database | RDS Postgres `db.t4g.micro`, single-AZ, encrypted, 7-day automated backups. Rejected Aurora Serverless v2 min=0 (cold-start UX bad for first-of-day form submitter); rejected DynamoDB (would lose the 6 saved SQL analytics queries). |
| D6 | API layer to replace Supabase auto-REST | 4 Lambda functions behind API Gateway HTTP API. Plain `psycopg` direct connections, no RDS Proxy in Phase 1 (~$15/mo saved; low traffic makes cold-connection cost negligible). |
...
| D10 | Observability + cost guardrail | CloudWatch Logs (default), one CloudWatch billing alarm at $20/mo. |
| D11 | Secrets / config | SSM Parameter Store (free tier for standard params) for env vars + DB creds. Injected into Lambda at runtime and into Amplify at build. |
...
```
           ...
           Amplify Hosting (CloudFront + S3, managed)
            ...
            CloudWatch Logs + 1 billing alarm ($20/mo)
            SSM Parameter Store (env + DB creds)
```
...
- **Lambda functions** — one per write endpoint: `POST /pilot-applications`, `POST /feature-requests`, `POST /engagement-signals`, `POST /votes`. Each ~30 lines of Python: Pydantic body validation, `psycopg` INSERT, return 201. Shared layer for DB connection + Pydantic models.
- **RDS Postgres** — single instance, 20 GB gp3 storage, deletion protection on, automated backups 7-day window (in addition to AWS Backup plan).
...
3. Create CloudWatch billing alarm at $20/mo on root account (email-only notification).
...
2. **RDS** — `db.t4g.micro`, 20 GB gp3, single-AZ, Postgres 16, deletion protection ON, automated backups 7-day. Public-access ON for Phase 3 only (laptop `pg_restore`); flipped OFF at end of Phase 3, never re-enabled. Master user `campuspandit_admin`, credentials in SSM `/campuspandit/prod/db_master_*`. Lives in the default VPC's subnets; security group `rds-sg` allows port 5432 ingress only from `lambda-sg` (defined below).
3. **API Gateway HTTP API** — create one API named `campuspandit-observe`. 4 routes, all POST, all anonymous. CORS: allow origin `https://www.campuspandit.ai` (+ Amplify preview URLs).
4. **Lambda** — 4 functions in Python 3.12, x86_64, 128 MB, 10s timeout. Shared Lambda layer containing `psycopg[binary]` + `pydantic`. Functions named `pilot-application-write`, `feature-request-write`, `engagement-signal-write`, `vote-write`. Attached to default VPC subnets with security group `lambda-sg` (no inbound; outbound to `rds-sg` on 5432). IAM role: `AWSLambdaVPCAccessExecutionRole`. Each function has env var `DATABASE_URL` populated at deploy time from SSM (Lambda does not call SSM at runtime — avoids needing a VPC endpoint and keeps cold-start fast).
5. **Amplify Hosting** — connect GitHub repo `spayyavula/campuspandit`, branch `main`, build command `npm ci && npm run build`, output dir `dist`. Add env var `VITE_API_BASE_URL` (pointing at the API Gateway invoke URL). Custom headers from §6.
6. **Cognito** — create empty user pool `campuspandit-users` in same region. Note the pool ID in SSM but don't wire it up.
7. **Route 53** — create hosted zone for `campuspandit.ai`. Do NOT change registrar nameservers yet (that's Phase 5). Pre-populate records: `campuspandit.ai` apex (placeholder), `www.campuspandit.ai` alias to Amplify CloudFront distribution.
8. **AWS Backup** — vault `campuspandit-prod`, plan `daily-35-monthly-glacier-365`, selection = the one RDS instance.
9. **ACM** — request a cert in `ap-south-1` for `www.campuspandit.ai` and `campuspandit.ai`. DNS-validate via Route 53 zone records (resolves immediately in same console).
...
3. Add a Postgres role `lambda_app` with INSERT-only privileges on these 4 tables; nothing else. Lambdas connect as `lambda_app`, not master.
...
| RDS t4g.micro | ~$13 | ~$13 (still well within limits) |
...
| **Total** | **~$16/mo** | **~$24-28/mo** |

Azure baseline (from the 2026-05-22 spec): ~$50-80/mo. Phase 1 cost reduction is the secondary benefit; primary is consolidation per [[b2b-pivot]] direction.
...
5. **No RDS Proxy** — Lambda cold start opens a fresh Postgres connection (~50-100ms). With <1k visits/mo and the natural infrequency of form submissions, connection storming is not a concern. If traffic grows past ~10 RPS sustained on form endpoints, add RDS Proxy.
...
- Steady-state monthly bill is ≤ $20.
...
- Multi-region or DR beyond AWS Backup (single-AZ RDS is acceptable for a side bet)
