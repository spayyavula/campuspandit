...
export default function Auth({ onAuthStateChange }: AuthProps) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-900 via-purple-900 to-pink-900 flex flex-col items-center justify-center relative overflow-hidden">
        <div className="relative z-10 w-full max-w-2xl mx-auto flex flex-col items-center text-center py-16 px-4">
          <div className="bg-white bg-opacity-10 backdrop-blur-lg rounded-2xl p-8 shadow-2xl">
            ...
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  const testEmail = `test+${Date.now()}@campuspandit.com`;
                  ...
                  console.log('Registration test:', { regData, regError });
                  
                  if (regData.user && !regError) {
                    console.log('✅ Registration works');
                    console.log('User created:', regData.user.id);
                    console.log('Session created:', regData.session ? 'Yes (auto-confirm)' : 'No (email confirm required)');
            ...
            >
              Debug Connection & Test Registration
            </button>
          </div>
        </div>
      </div>
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    console.log('Auth attempt:', { mode, email: form.email });
    ...
    try {
      if (mode === 'register') {
        ...
        // Check the registration result to understand email confirmation status
        if (data.user && data.session) {
          // Auto-confirmation is enabled (email confirmation disabled)
          console.log('Auto-confirmed registration, user logged in immediately');
          setMessage(
            <div className="text-center">
              <p className="mb-2">🎉 Registration successful!</p>
              <p className="mb-3">✅ You are now logged in and can access the dashboard.</p>
              <p className="text-sm text-green-300">
                Email verification is disabled - you're automatically confirmed.
              </p>
            </div>
          );
          ...
          if (data.user.confirmation_sent_at) {
            setMessage(
              <div className="text-center">
                <p className="mb-2">✅ Registration successful!</p>
                <p className="mb-3">📧 A verification email has been sent to <strong>{form.email}</strong></p>
                <p className="text-sm mb-3">Please check your email (including spam folder) and click the verification link.</p>
                <button
                  type="button"
                  onClick={async () => {
                    try {
            ...
            // Email was not sent - this indicates a configuration issue
            setMessage(
              <div className="text-center">
                <p className="mb-2">⚠️ Registration completed but verification email was not sent.</p>
                <p className="text-sm mb-3">This usually indicates an email configuration issue.</p>
                <div className="space-y-2">
                      ...
                      try {
                        ...
                        if (error) {
                          if (error.message.includes('rate limit')) {
                            setMessage('⏱️ Rate limited. Please wait a moment before trying again.');
                          } else if (error.message.includes('not found')) {
                            setMessage('❌ User not found. Please try registering again.');
                          } else {
                            setMessage(`❌ Email service error: ${error.message}`);
                          }
                        } else {
                        ...
                        console.error('Resend error:', err);
                        setMessage('❌ Failed to send verification email. Please contact support.');
                      }
                    }}
                  ...
                  <button
                    ...
                    onClick={() => {
                      setMode('login');
                      setMessage('You can try logging in if email confirmation is not required.');
                    }}
          ...
          setTimeout(() => {
            setMode('login');
            setMessage('You can now try logging in with your credentials.');
          }, 10000); // 10 seconds
        } else {
          console.log('Unexpected registration state:', data);
          setMessage('Registration completed but status is unclear. Please try logging in.');
        }
        success = true;
      } else if (mode === 'login') {
        ...
        if (error) {
          ...
          if (error.message.includes('Invalid login credentials')) {
            setMessage(
              <div className="text-center">
                <p className="mb-2">Invalid login credentials. This could mean:</p>
                <ul className="text-sm text-left mb-3 space-y-1">
                  <li>• Email address is not registered</li>
                  <li>• Password is incorrect</li>
                  <li>• Email address is not verified yet</li>
                </ul>
                <div className="space-y-2">
                  <button
                    type="button"
                      ...
                      try {
                        ...
                        if (error) {
                          if (error.message.includes('not found')) {
                            setMessage('Email not found. Please register first or check your email address.');
                          } else {
                            throw error;
                          }
                        } else {
            ...
            setMessage(
              <div className="text-center">
                <p className="mb-2">Please check your email and click the verification link before logging in.</p>
                <p className="text-sm mb-3">Check your spam folder if you don't see the email.</p>
                <button
                  type="button"
                  onClick={async () => {
                    try {
      ...
      setLoading(false);
      // Only hide login modal after successful login
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-purple-900 to-pink-900 flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto flex flex-col items-center text-center py-16 px-4 md:px-0">
        ...
        {/* Email Troubleshooter - for email issues */}
        <EmailTroubleshooter />
      </div>
    </div>
  );
